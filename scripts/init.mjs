...
const titleCase = projectName.replace(/[-_]+/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())

const envFiles = ['.env', '.env.example', '.env.development', '.env.production']
const targets = [
  {
    file: 'package.json',
    transform: (c) => c.replace(/"name":\s*"vue3-starter"/, `"name": "${projectName}"`),
  },
  {
    file: 'index.html',
    transform: (c) => c.replace(/<title>vue3-starter<\/title>/, `<title>${titleCase}</title>`),
  },
